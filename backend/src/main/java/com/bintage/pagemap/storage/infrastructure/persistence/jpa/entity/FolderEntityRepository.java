...
public interface FolderEntityRepository extends JpaRepository<FolderEntity, Long> {
    ...
    @Query("SELECT f FROM FolderEntity f WHERE f.accountId = :accountId AND f.parentFolderId = :parentFolderId AND f.delete.moveTrashed = true")
    List<FolderEntity> findDeletedAllByParentFolderId(String accountId, Long parentFolderId);
    ...
    @Query("SELECT f FROM FolderEntity f LEFT JOIN FETCH f.childrenFolder cf LEFT JOIN FETCH f.childrenBookmark  cb WHERE f.accountId = :accountId AND f.id = :id AND f.delete.moveTrashed = true")
    Optional<FolderEntity> findFetchDeletedFamilyById(String accountId, Long id);
    ...
    @Query("SELECT f FROM FolderEntity f WHERE f.accountId = :accountId AND f.id IN :ids AND f.delete.moveTrashed = true")
    List<FolderEntity> findDeletedAllById(String accountId, Set<Long> ids);
